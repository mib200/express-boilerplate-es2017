...
const { some, omitBy, isNil } = require('lodash'); // eslint-disable-line
...
describe('Users API', async () => {
  ...
  describe('GET /v1/users/:userId', () => {
    ...
    it('should report error when logged user is not the same as the requested one', async () => {
      const id = (await User.findOne({ email: dbUsers.branStark.email })).id;
  ...
  describe('PUT /v1/users/:userId', () => {
    it('should replace user', async () => {
      delete dbUsers.branStark.password;
      const id = (await User.findOne(dbUsers.branStark)).id;
    ...
    it('should not replace the role of the user (not admin)', async () => {
      const id = (await User.findOne({ email: dbUsers.jonSnow.email })).id;
      const role = 'admin';
